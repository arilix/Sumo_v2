...
void loop() {
  if (Ps3.isConnected()) {
    ...
    // Jika tombol segitiga ditekan, hentikan robot dan abaikan input lain.
    if (Ps3.data.button.triangle) {
      stopped();
      Serial.println("Robot Terkunci (Segitiga Ditekan)");
    } else {
      // Jika segitiga tidak ditekan, jalankan logika gerakan normal.
      bool isMoving = false;
      int speed;
