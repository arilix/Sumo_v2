...
// ========== KONFIGURASI PIN (Disesuaikan dengan soccer.txt) ==========
int ena_kanan = 26;
int ena_kiri = 33;
int RPWM_kanan = 13;
int LPWM_kanan = 15;
...
// ========== FUNGSI GERAKAN (Logika disesuaikan dengan soccer.txt) ==========
void kanan() {
...
// Fungsi gerakan serong (Disesuaikan dengan logika baru)
void maju_kiri() {
  ledcWrite(pwm_channel, 0);
...
void loop() {
  if (Ps3.isConnected()) {
    ...
    if (ly < -50 && lx < -50) {
      ...
      maju_kiri();
      Serial.print("L3: maju kiri"); Serial.println(speed_mode_text); // DIUBAH
      isMoving = true;
    } else if (ly < -50 && lx > 50) {
      pwm_ki = speed; pwm_ka = speed;
      maju_kanan();
      Serial.print("L3: maju kanan"); Serial.println(speed_mode_text); // DIUBAH
      isMoving = true;
    } else if (ly > 50 && lx < -50) {
      pwm_ki = speed; pwm_ka = speed;
      mundur_kiri();
      Serial.print("L3: mundur kiri"); Serial.println(speed_mode_text); // DIUBAH
      isMoving = true;
    } else if (ly > 50 && lx > 50) {
      pwm_ki = speed; pwm_ka = speed;
      mundur_kanan();
      Serial.print("L3: mundur kanan"); Serial.println(speed_mode_text); // DIUBAH
      isMoving = true;
    }
    // Gerakan lurus
    else if (ly < -50) {
      pwm_ki = speed; pwm_ka = speed;
      maju();
      Serial.print("L3: maju"); Serial.println(speed_mode_text); // DIUBAH
      isMoving = true;
    } else if (ly > 50) {
      pwm_ki = speed; pwm_ka = speed;
      mundur();
      Serial.print("L3: mundur"); Serial.println(speed_mode_text); // DIUBAH
      isMoving = true;
    } else if (lx < -50) {
      pwm_ki = speed; pwm_ka = speed;
      kiri();
      Serial.print("L3: kiri"); Serial.println(speed_mode_text); // DIUBAH
      isMoving = true;
    } else if (lx > 50) {
      pwm_ki = speed; pwm_ka = speed;
      kanan();
      Serial.print("L3: kanan"); Serial.println(speed_mode_text); // DIUBAH
      isMoving = true;
    }
    ...
    if (!isMoving) {
        ...
        if (Ps3.data.button.up && Ps3.data.button.left) {
            pwm_ki = speed; pwm_ka = speed;
            maju_kiri();
            Serial.print("D-pad: maju kiri"); Serial.println(speed_mode_text); // DIUBAH
            isMoving = true;
        } else if (Ps3.data.button.up && Ps3.data.button.right) {
            pwm_ki = speed; pwm_ka = speed;
            maju_kanan();
            Serial.print("D-pad: maju kanan"); Serial.println(speed_mode_text); // DIUBAH
            isMoving = true;
        } else if (Ps3.data.button.down && Ps3.data.button.left) {
            pwm_ki = speed; pwm_ka = speed;
            mundur_kiri();
            Serial.print("D-pad: mundur kiri"); Serial.println(speed_mode_text); // DIUBAH
            isMoving = true;
        } else if (Ps3.data.button.down && Ps3.data.button.right) {
            pwm_ki = speed; pwm_ka = speed;
            mundur_kanan();
            Serial.print("D-pad: mundur kanan"); Serial.println(speed_mode_text); // DIUBAH
            isMoving = true;
        }
        // Gerakan lurus D-pad
            ...
            pwm_ki = speed; pwm_ka = speed;
            maju();
            Serial.print("D-pad: maju"); Serial.println(speed_mode_text); // DIUBAH
            isMoving = true;
        } else if (Ps3.data.button.down) {
            pwm_ki = speed; pwm_ka = speed;
            mundur();
            Serial.print("D-pad: mundur"); Serial.println(speed_mode_text); // DIUBAH
            isMoving = true;
        } else if (Ps3.data.button.left) {
            pwm_ki = speed; pwm_ka = speed;
            kiri();
            Serial.print("D-pad: kiri"); Serial.println(speed_mode_text); // DIUBAH
            isMoving = true;
        } else if (Ps3.data.button.right) {
            pwm_ki = speed; pwm_ka = speed;
            kanan();
            Serial.print("D-pad: kanan"); Serial.println(speed_mode_text); // DIUBAH
            isMoving = true;
        }
    }
    ...
    if (!isMoving) {
      stopped();
      Serial.println("stop"); // DIUBAH
    }
  } else {
    stopped();
    Serial.println("PS3 Disconnected - STOP"); // DIUBAH
  }
}